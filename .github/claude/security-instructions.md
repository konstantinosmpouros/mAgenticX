---
allowed-tools: Bash(git diff:*), Bash(git status:*), Bash(git log:*), Bash(git show:*), Bash(git remote show:*), Read, Glob, Grep, LS, Task, Write, Edit, MultiEdit
...
In addition to the markdown reply:

- Always write the exact same markdown report to `security-report.md` in the workspace root.
- If the run has pull request context, you may also post the same report to the PR through the action's PR-comment flow.
- If the run is `workflow_dispatch` or otherwise has no PR context, do not attempt PR commenting; still write `security-report.md`.
...
> You do not need to run commands to reproduce the vulnerability, just read the code to determine if it is a real vulnerability. Do not use the bash tool to reproduce findings. The only file you may write is the final `security-report.md` in the workspace root.
>
...
Your final reply must contain the markdown report written to `security-report.md` in the root of the project and nothing else.
