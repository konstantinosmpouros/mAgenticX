...
const sharedTooltipClass =
  "!opacity-100 bg-background text-foreground border border-border shadow-card px-2 py-1 rounded-md";
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <MetricCard
          ...
          value={fmtDate(
            latestArchivedConversation?.archivedAt ?? latestArchivedConversation?.updated_at,
          )}
          ...
          <div className="space-y-3 p-4">
            {archivedConversations.length === 0 && !archivedConversationsLoading ? (
              <SoftPanel className="px-4 py-10 text-center">
                <p className="text-sm text-muted-foreground">No archived conversations yet.</p>
              </SoftPanel>
            ) : (
