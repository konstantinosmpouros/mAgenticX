...
from core.logging.redaction import sanitize_for_logging
...
class JsonFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        payload = {
            ...
            "service": getattr(record, "service", "agents"),
            ...
            "agent_slug": getattr(record, "agent_slug", None),
        ...
        if isinstance(sanitized, dict):
            for key in (
                ...
                if value not in (None, "", {}):
                    if key not in {"user_id", "session_id", "conversation_id", "message_id", "agent_slug", "thread_id"}:
