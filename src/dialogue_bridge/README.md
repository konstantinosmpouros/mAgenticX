...
- Upsert Vault-authenticated users, seed active agents, and expose a roster that stays in sync with the agents service.
...
- `AgentTable` caches metadata and endpoints for LangGraph agents (seeded on startup).
...
Schema creation and agent seeding run inside the FastAPI lifespan hook at startup.
