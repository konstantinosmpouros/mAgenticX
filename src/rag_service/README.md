...
- Loads Excel workbooks from `data/` into in-memory DuckDB tables at startup (first worksheet only, snake_case table names). Startup fails fast if no sheets load.
...
- `RAG_HOST`, `RAG_PORT` – Chroma host/port (defaults match the Compose `vectordb` service).
- Excel files must exist under `data/` before startup.
