...
export default function ShortcutsTab() {
  ...
  const shortcutSections = (["Workspace", "Chat", "Composer", "Dismiss"] as ShortcutCategory[]).map(
    (category) => ({
      category,
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <SoftPanel className="flex-1 p-4">
            ...
            <p className="mt-2 text-sm text-foreground">
              `Esc` dismisses the top active app surface first: image preview, profile panel, agent
              picker, conversation action menus, sidebar rename or action menus, then inline message
              editing. It never stops inference or voice dictation.
            </p>
          </SoftPanel>
          ...
          <section key={category} className="space-y-3">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {category === "Chat" &&
                  "Conversation-level actions that affect the current chat shell."}
                {category === "Composer" &&
                  "Composer-local keys handled directly inside the input."}
