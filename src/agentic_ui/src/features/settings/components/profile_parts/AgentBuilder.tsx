    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className={labelClass}>
          ...
          <span className="text-[0.7rem] text-muted-foreground">
            Identifier: <code className="font-mono">{draft.slug || "—"}</code>
          ...
          {ICON_CHOICES.map((name) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "rounded-xl border px-2.5 py-1.5 text-xs transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60",
                  active
      ...
      <div className="flex flex-col gap-2">
        ...
        {mySkills.length === 0 ? (
          <p className="text-xs text-muted-foreground">
            Your skill pool is empty. Add skills under Settings → Skills, then assign them here.
          </p>
          ...
          <div className="flex flex-col gap-3">
            {draft.subagents.map((sa, index) => (
              <SoftPanel key={index} className="flex flex-col gap-3 px-4 py-3">
                ...
                <textarea
                  className={cn(inputClass, "min-h-[5rem] resize-y font-mono text-[0.78rem]")}
                  value={sa.prompt}
                  onChange={(event) => updateSubagent(index, { prompt: event.target.value })}
      ...
      >
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[0.7rem] text-muted-foreground">
              {generatedCount + draft.files.length} / {MAX_FILES} · {formatBytes(totalBytes)}
            </span>
            <button
              type="button"
        ...
        <p className="text-xs text-muted-foreground">
          Optional notes, checklists or examples the agent can read while it works — drop files here
          or add a path like <code className="font-mono">notes/style.md</code>. Text only (
          {FILE_EXTENSIONS.join(", ")}). Refer to them from your instructions by their full path,
          shown next to each file.
        </p>
            ...
            {draft.files.map((file) => {
              ...
              return (
                <div key={file.path} className="flex flex-col gap-1.5">
                  ...
                  {isActive ? (
                    <textarea
                      className={cn(inputClass, "min-h-[7rem] resize-y font-mono text-[0.78rem]")}
                      value={file.content}
                      onChange={(event) => updateFileContent(file.path, event.target.value)}
      ...
      <div className="flex items-center justify-end gap-2">
        <button
          type="button"
          onClick={onClose}
          className="rounded-xl border border-border/60 bg-background/60 px-3 py-2 text-sm text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
        >
          Cancel
        </button>
        <button
          ...
          disabled={!canSubmit}
          className={cn(
            "inline-flex items-center gap-2 rounded-xl border border-primary/40 bg-primary/15 px-3.5 py-2 text-sm font-medium text-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60",
