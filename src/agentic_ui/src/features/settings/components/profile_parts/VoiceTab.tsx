...
export default function VoiceTab({
  ...
  const selectedLanguage =
    VOICE_MODE_LANGUAGES.find((language) => language.id === voiceModeLanguage) ??
    VOICE_MODE_LANGUAGES[0];
  ...
  const handlePreviewVoice = async (voice: RealtimeVoice) => {
    ...
    try {
      const audioBlob = await generateReadAloudPreviewAudio(
        user.id,
        voice,
        "Hey! I am your AI speaker.",
      );
