...
| `present_artifact` tool | [src/agents/harness/tools/present_artifact.py](../../src/agents/harness/tools/present_artifact.py) | `build_present_artifact_tool()` — path-guarded, returns a confirmation (never emits); registered in `harness/tools/registry.py` as an auto-attach native |
