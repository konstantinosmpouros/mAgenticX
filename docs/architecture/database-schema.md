...
| `enabled_tools` | `JSON` | Yes | `NULL` | Tool list snapshot — `enabledTools` is client-computed, so a headless fire carries its own frozen list |
...
- **Schema changes go through Alembic migrations.** Edit the ORM model in `core/database/models.py`, then autogenerate + hand-review a migration in `migrations/versions/` (the ordered chain `0001_baseline` → `0007_…`). `alembic upgrade head` runs automatically on bridge startup. Never use `metadata.create_all`. See the CLAUDE.md "Schema Changes — Migration Workflow" section.
