...
- **Schema changes go through Alembic migrations.** Edit the ORM model in `core/database/models.py`, then autogenerate + hand-review a migration in `migrations/versions/` (the ordered chain `0001_baseline` → `0016_retire_enabled_tools`). `alembic upgrade head` runs automatically on bridge startup. Never use `metadata.create_all`. See the CLAUDE.md "Schema Changes — Migration Workflow" section.
