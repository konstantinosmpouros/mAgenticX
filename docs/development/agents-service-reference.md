...
- Opens `AsyncConnectionPool(conninfo=…, min/max/idle/timeout from settings, open=False)` with `conn_kwargs={autocommit:True, row_factory:dict_row, prepare_threshold:None}` (pgbouncer-safe), then `pool.open()` + `pool.wait()`. Stored at `app.state.checkpointer_pool`.
