...
| Memory store ops (list/read/delete + row format) | [src/agents/harness/filesystem/memory.py](../../src/agents/harness/filesystem/memory.py) | `index_line()` / `index_line_pattern()`, `list_memories()`, `read_memory()`, `delete_memory()` |
...
| AG-UI event emitter | [src/agents/harness/protocols/agui/emitter.py](../../src/agents/harness/protocols/agui/emitter.py) | `AGUIEmitter` — all emit methods |
| AG-UI normalizer | [src/agents/harness/protocols/agui/normalizer.py](../../src/agents/harness/protocols/agui/normalizer.py) | `AGUIStreamNormalizer.handle_chunk()` |
| Custom event types | [src/agents/harness/protocols/agui/events.py](../../src/agents/harness/protocols/agui/events.py) | `PlanItem`, `PlanSnapshot`, `TaskSubAgentEvent`, `HITLInterruptEvent` |
