...
The helpers are `user_root` (provisioner.py:89-95), `memory_root` (98-105), `agent_root` (118-125), `skills_root` (128-130), `conversation_root` (133-140), and `ensure_user_agent_filesystem` (252-300). Mounts are assembled by `build_workspace_backend` ([runtime/filesystem/workspace.py:51-157](../../src/agents/runtime/filesystem/workspace.py), routes at workspace.py:128-154) with the write-deny ladder at workspace.py:42-48. The deep agent calls it with exactly three identity values — `user_id`, `agent_slug`, `conversation_id` ([runtime/deep_agent.py:258-262](../../src/agents/runtime/deep_agent.py)) — and `load_agent_md` (deep_agent.py:501-519) returns `["/memories/AGENTS.md"]` when memory is on.

**The run context is a loose dict**, which is what makes threading a fourth key cheap: `Request.config` is `Dict[str, Any]` ([schemas.py:6-9](../../src/agents/schemas.py)), `self.context = self.config.get("context", {})` ([runtime/base_agent.py:73](../../src/agents/runtime/base_agent.py)), and `_validate_context_config` (base_agent.py:200-210) requires exactly `user_id` and `conversation_id` to be non-empty strings. Adding `workspace_id` is one entry in that required-key tuple.
...
| `global_root` (settings.py:455-458, `MAGENTICX_GLOBAL_ROOT`) | `/var/magenticx/global` | **partly live** — `_scan_yaml_agents` reads `<global_root>/agents` ([utils/agents.py:126](../../src/agents/utils/agents.py)) and `seed_global_agents` writes it ([runtime/declarative/agent_seed.py:43](../../src/agents/runtime/declarative/agent_seed.py)) |
...
| Run context (needs `workspace_id`) | [src/agents/runtime/base_agent.py](../../src/agents/runtime/base_agent.py) · [src/agents/schemas.py](../../src/agents/schemas.py) | `self.context` 73, `use_memory` 79, `personalization` 86, `_validate_context_config` 200-210 (required keys 205) · `Request` 6-9 |
| Deep-agent wiring | [src/agents/runtime/deep_agent.py](../../src/agents/runtime/deep_agent.py) | `_build_composite_backend` 245-262, `load_agent_md` 501-519, permissions applied 465 |
...
| Global YAML agent seed + scan | [src/agents/runtime/declarative/agent_seed.py](../../src/agents/runtime/declarative/agent_seed.py) · [src/agents/utils/agents.py](../../src/agents/utils/agents.py) | `seed_global_agents` 35+ (target 43) · `_scan_yaml_agents` 78-115, called at 126 |
