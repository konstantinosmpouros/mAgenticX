...
> **Depends on:** [00-platform-restructure.md](done/00-platform-restructure.md) (**Done** — it established the declared-tool superset). Soft: [10-rag-via-mcp-gateway.md](10-rag-via-mcp-gateway.md) (more tools arriving through the gateway is what makes this urgent).
...
**Other plans.** [10-rag-via-mcp-gateway.md](10-rag-via-mcp-gateway.md) is the reason this plan matters: moving Chroma retrieval behind the gateway multiplies the MCP catalog, and its tools' descriptions become index rows automatically. [06-deep-research-mode.md](06-deep-research-mode.md) is the first consumer with a genuinely wide tool surface. [12-create-skill-tool.md](done/12-create-skill-tool.md) is adjacent but distinct — skills are already progressively disclosed by the deepagents `SkillsMiddleware`, which is the same idea one layer up and a useful precedent for "the model asks for detail on demand". [01-custom-agents-per-user.md](done/01-custom-agents-per-user.md) multiplies the number of distinct authorized sets, which is fine here because the index is global and only the query-time intersection is per-(user, agent).
