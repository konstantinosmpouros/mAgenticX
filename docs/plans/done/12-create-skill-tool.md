...
> **Status:** **Delivered** — the tool, its provenance stamping and the multi-file skill folder are in and confirmed working by the user; the TODO item was closed on 2026-08-29. Two sections here were deliberately **not** built and were never requested: §3.6 per-user quotas and §3.8 the review/revoke surface. The bridge's Redis skill cache is also not invalidated on a tool-created skill, so it can take up to 2 h (or a manual refresh) to appear in Settings.
>
> **What shipped:** the tool itself (`harness/tools/create_skill.py`), registered
...
Roots are `FilesystemSettings` fields: `user_root` at [`src/agents/core/settings.py:426`](../../../src/agents/core/settings.py), `skills_registry_global_root` at `:434`, `skills_registry_users_root` at `:442`. The subsystem's own docstring states the split ([`src/agents/harness/skill_registry/__init__.py`](../../../src/agents/harness/skill_registry/__init__.py)).
...
[`src/agents/harness/filesystem/provisioner.py:372`](../../../src/agents/harness/filesystem/provisioner.py):
...
**Nothing in the manifest records where a skill came from.** `SkillManifestEntry` ([`src/agents/schema/__init__.py:190-206`](../../../src/agents/schema/__init__.py)) is `name`, `type: Literal["global","custom"]`, `description`, `source_path`, `category`. A user-uploaded custom skill and an agent-authored one would be indistinguishable — which is the first thing this plan has to change.
...
The schema change is to the **on-disk manifest**, in [`src/agents/schema/__init__.py:190`](../../../src/agents/schema/__init__.py):
...
The bridge and UI contracts carry the fields through: `UserSkill` / `UserSkillDetail` in [`schemas/__init__.py:162`, `:192`](../../../src/dialogue_bridge/schema/__init__.py), and `toUserSkill` in `shared/lib/schemas.ts:71` (which is field-whitelisted — a new backend field is silently dropped client-side unless it is added there).
...
**Phase 2 — The tool, gated off.** Add `harness/tools/create_skill.py` and register it `auto_attach=False`; extend `NativeToolContext` with `allow_skill_authoring`; implement validation, quotas, the pool write, the enable step, and the model-legible returns. No agent declares it yet and the preference defaults off, so it cannot be reached.
...
| --- | --- | --- |
| Native-tool registry (where `create_skill` registers) | [src/agents/harness/tools/registry.py](../../../src/agents/harness/tools/registry.py) | `NativeToolContext` `:31`, `NativeToolDef` `:44`, `register_native_tool` `:70`, `resolve_native_tool` `:141`, `is_known_native_tool` `:136`, `native_catalog` `:162` |
| The model for a durable-write native tool | [src/agents/harness/tools/remember.py](../../../src/agents/harness/tools/remember.py) | `build_remember_tool` `:90`, `_slugify` `:59`, `_atomic_write` `:68`, caps `:39-40`, entry cap `:113-120` |
| User-pool registry — the write path to reuse | [src/agents/harness/skill_registry/user_registry.py](../../../src/agents/harness/skill_registry/user_registry.py) | `add_custom_to_user` `:382`, `_assemble_skill_md` `:375` (**the fix**), `_validate_skill_relpath` `:93`, `_decode_skill_file` `:119`, caps `:79-82`, `_write_user_manifest_atomic` `:206`, `resolve_skill_path` `:284`, `reconcile_user_manifest` `:531` |
| Enable / disable / cascade | [src/agents/harness/skill_registry/user_registry.py](../../../src/agents/harness/skill_registry/user_registry.py) · [harness/filesystem/provisioner.py](../../../src/agents/harness/filesystem/provisioner.py) | `_enable_skill_for_agent` `:623` (`copytree` `:646`), `assign_user_skill_to_agent` `:657`, `_cascade_remove_from_agent_assignments` `:475` · `list_enabled_skills` `:372`, `disable_skill` `:385`, `_safe_segment` `:70` |
| Manifest schema to extend | [src/agents/schema/__init__.py](../../../src/agents/schema/__init__.py) | `SkillManifestEntry` `:190`, `UserManifest` `:216`, `CustomSkillCreate` `:240`, `SkillFile` `:224` |
| Global catalog | [src/agents/harness/skill_registry/global_manifest.py](../../../src/agents/harness/skill_registry/global_manifest.py) · [seed_global_registry.py](../../../src/agents/harness/skill_registry/seed_global_registry.py) | `is_global_skill` (collision check), `rebuild_global_manifest`, `seed_global_registry` `:40` |
| Skills mount + write-deny | [src/agents/harness/filesystem/workspace.py](../../../src/agents/harness/filesystem/workspace.py) · [harness/abstractions/deep_agent.py](../../../src/agents/harness/abstractions/deep_agent.py) | `/skills/` route `:134`, `WORKSPACE_WRITE_DENY` `:43` · `load_skills` `:476-488`, `create_deep_agent(skills=…)` `:463` |
| Forced HITL injection point | [src/agents/harness/abstractions/deep_agent.py](../../../src/agents/harness/abstractions/deep_agent.py) · [harness/abstractions/agent_spec.py](../../../src/agents/harness/abstractions/agent_spec.py) | `build_deep_agent` `:387` · `hitl` field `:145`, wired at [yaml_agent.py:158](../../../src/agents/harness/abstractions/yaml_agent.py) |
...
| Bridge schemas | [src/dialogue_bridge/schema/__init__.py](../../../src/dialogue_bridge/schema/__init__.py) | `UserSkill` `:162`, `UserSkillDetail` `:192`, `CustomSkillCreateRequest` `:228` (`max_length=30` `:237`) |
