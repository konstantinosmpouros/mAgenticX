...
> **Blocks:** nothing. [12 · `create_skill` tool](done/12-create-skill-tool.md) is scoped to markdown-only skills specifically because execution does not exist yet; this plan is what would let an agent-authored skill ever carry a script
...
**`execute` is a reserved tool name.** `RESERVED_DEEPAGENT_TOOL_NAMES` at [`src/agents/harness/abstractions/deep_agent.py:37-58`](../../src/agents/harness/abstractions/deep_agent.py) includes `"execute"` at `:48`, so a live MCP tool named `execute` is dropped rather than smuggled in under the framework's name.
...
Approval is not a greenfield build. deepagents' `interrupt_on` is already wired both ways: declaratively, via the `agent.yaml` `hitl:` map ([`src/agents/harness/abstractions/agent_spec.py:145`](../../src/agents/harness/abstractions/agent_spec.py), passed at [`yaml_agent.py:158`](../../src/agents/harness/abstractions/yaml_agent.py)), and in Python via `HITL_GATED_TOOLS` at [`src/agents/deep_agents/omni_agent/__init__.py:14-23`](../../src/agents/deep_agents/omni_agent/__init__.py), passed as `interrupt_on=HITL_GATED_TOOLS` at `:91`. **Both already list `execute: true`** — the seeded YAML agent at [`src/agents/harness/agents_seed/omni-yaml-v1/agent.yaml:37-41`](../../src/agents/harness/agents_seed/omni-yaml-v1/agent.yaml) and Omni's Python map, whose comment reads "Code execution — arbitrary shell / python is always user-approved". The interrupt reaches the UI as `HITL_INTERRUPT` and the decision comes back through `POST /v1/inference/runs/{user_id}/{run_id}/resume` ([`src/dialogue_bridge/router/inference.py:225`](../../src/dialogue_bridge/router/inference.py)) → `request_run_resume` ([`utils/inference_runs.py:1482`](../../src/dialogue_bridge/utils/inference_runs.py)) → `POST /agents/{slug}/resume` ([`src/agents/router/inference.py:131`](../../src/agents/router/inference.py)), which feeds `Command(resume=...)` into the saved LangGraph checkpoint.
...
**Authorization is identity-from-the-run, never identity-from-the-sandbox.** The backend is bound per run to `(user_id, agent_slug, conversation_id)` at build time — the same containment pattern as `build_remember_tool` (`harness/tools/remember.py:90`) — and the runner resolves paths from those values, so nothing the sandbox says can widen its own scope. Cross-tenant reach is escape-test 7 rather than an assumption.
...
| --- | --- | --- |
| Write-deny ladder + mount routes | [src/agents/harness/filesystem/workspace.py](../../src/agents/harness/filesystem/workspace.py) | `WORKSPACE_WRITE_DENY` `:42-48` (input deny `:47`), `build_workspace_backend` `:51`, routes `:128-154` |
| The fail-closed sandbox guard | [src/agents/harness/filesystem/workspace.py](../../src/agents/harness/filesystem/workspace.py) | `factory` `:111-127` — `RuntimeError` when a sandbox-capable default meets a false flag |
...
| Permissions applied to every deep agent | [src/agents/harness/abstractions/deep_agent.py](../../src/agents/harness/abstractions/deep_agent.py) | `build_deep_agent` `:387`, `permissions=list(WORKSPACE_WRITE_DENY)` `:465` |
| `execute` as a reserved name | [src/agents/harness/abstractions/deep_agent.py](../../src/agents/harness/abstractions/deep_agent.py) | `RESERVED_DEEPAGENT_TOOL_NAMES` `:37-58`, `"execute"` `:48` |
...
| Retention sweeper | [src/agents/harness/filesystem/retention.py](../../src/agents/harness/filesystem/retention.py) | budgets `:49-50`, activity grace `:52`, symlink removal `:157-166`, containment refusal `:213-225`, loop `:234` |
...
| HITL gate, declarative | [src/agents/harness/agents_seed/omni-yaml-v1/agent.yaml](../../src/agents/harness/agents_seed/omni-yaml-v1/agent.yaml) · [harness/abstractions/agent_spec.py](../../src/agents/harness/abstractions/agent_spec.py) | `hitl:` `:37-41` · `hitl` field `:145`, wired at [yaml_agent.py:158](../../src/agents/harness/abstractions/yaml_agent.py) |
...
| Per-run identity binding pattern | [src/agents/harness/tools/remember.py](../../src/agents/harness/tools/remember.py) | `build_remember_tool` `:90` — closure over `(user_id, agent_slug, conversation_id)` |
