...
| --- | --- | --- |
| `present_artifact` tool (existing) | [src/agents/harness/tools/present_artifact.py](../../src/agents/harness/tools/present_artifact.py) | `build_present_artifact_tool`, `resolve_output_file` guard, `_MAX_TITLE` / `_MAX_SUMMARY` |
| Native registry (existing) | [src/agents/harness/tools/registry.py](../../src/agents/harness/tools/registry.py) | `present_artifact` registration at `:117-132`, `auto_attach=True`, `build_auto_attach_tools` |
| AG-UI event models | [src/agents/harness/agui/events.py](../../src/agents/harness/agui/events.py) | `PresentArtifactEvent` (`:112-129`); add `ARTIFACT_OPEN/DELTA/CLOSE` types + models |
| Emitter | [src/agents/harness/agui/emitter.py](../../src/agents/harness/agui/emitter.py) | `present_artifact()` (`:340`); add the three artifact-draft methods |
| Normalizer synthesis | [src/agents/harness/agui/normalizer.py](../../src/agents/harness/agui/normalizer.py) | `if tc_name == "present_artifact"` (`:386-418`); add the `write_file`/`edit_file`-under-`output/` branch |
| Filesystem mounts + guards | [src/agents/harness/filesystem/provisioner.py](../../src/agents/harness/filesystem/provisioner.py) | layout docstring (`:1-56`), `conversation_output_root`, `resolve_output_file`, `read_output_files` (`:187`); add `seed_output_files` |
| `output/` TTL sweeper | [src/agents/harness/filesystem/retention.py](../../src/agents/harness/filesystem/retention.py) | "copies of DB-owned data" rationale (`:1-9`), `_ACTIVITY_GRACE_SECONDS` |
